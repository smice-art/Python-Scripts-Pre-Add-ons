...
def _merge_by_prefix(context, collection_name, prefix, merged_name):
    """Manually rebuild a single curve object out of several source curve objects,
    copying each spline AND explicitly remapping its material — bpy.ops.object.join()
    does not reliably preserve per-spline materials for legacy Curve objects, so we
    don't use it here."""
    collection = bpy.data.collections.get(collection_name)
    if collection is None:
        return None, "Collection not found"

    src_objs = [o for o in collection.objects if o.name.startswith(prefix) and o.type == 'CURVE']
    if len(src_objs) < 2:
        return None, f"Need at least 2 '{prefix}*' curve objects to merge"

    src0 = src_objs[0].data
    new_curve_data = bpy.data.curves.new(merged_name, type='CURVE')
    new_curve_data.dimensions = '3D'
    new_curve_data.resolution_u = src0.resolution_u
    new_curve_data.bevel_depth = src0.bevel_depth
    new_curve_data.bevel_resolution = src0.bevel_resolution
    new_curve_data.fill_mode = src0.fill_mode

    merged_obj = bpy.data.objects.new(merged_name, new_curve_data)
    collection.objects.link(merged_obj)

    for src_obj in src_objs:
        src_data = src_obj.data
        for spline in src_data.splines:
            # find (or register) this spline's material in the merged object's slot list
            mat = None
            if src_data.materials:
                idx = spline.material_index if spline.material_index < len(src_data.materials) else 0
                mat = src_data.materials[idx]

            if mat is not None:
                existing_names = [m.name for m in new_curve_data.materials]
                if mat.name not in existing_names:
                    new_curve_data.materials.append(mat)
                    mat_index = len(new_curve_data.materials) - 1
                else:
                    mat_index = existing_names.index(mat.name)
            else:
                mat_index = 0

            new_spline = new_curve_data.splines.new(spline.type)
            ...
            if spline.type == 'BEZIER':
                ...
                for i, p in enumerate(spline.bezier_points):
                    co = src_obj.matrix_world @ p.co
                    hl = src_obj.matrix_world @ p.handle_left
                    hr = src_obj.matrix_world @ p.handle_right
                    np = new_spline.bezier_points[i]
                    np.co, np.handle_left, np.handle_right = co, hl, hr
            else:
                new_spline.points.add(len(spline.points) - 1)
                for i, p in enumerate(spline.points):
                    co = src_obj.matrix_world @ Vector((p.co.x, p.co.y, p.co.z))
                    new_spline.points[i].co = (co.x, co.y, co.z, p.co.w)

            new_spline.material_index = mat_index

    # clean up the now-redundant source objects (their data too, if unused elsewhere)
    for src_obj in src_objs:
        data = src_obj.data
    ...
    # Colors now live per-spline as materials; switch any visible 3D viewport to
    # show material colors, since a single object can only have one "Object Color".
    if context.screen:
