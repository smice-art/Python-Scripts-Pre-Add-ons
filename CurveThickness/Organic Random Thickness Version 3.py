...
IMPORTANT, works from Blender 4.5 LTS
...
def build_interface(node_group):
    ...
    s = iface.new_socket("Resample Count", in_out='INPUT', socket_type='NodeSocketInt')
    s.default_value = 180
    s.min_value = 2
...
def build_nodes(node_group):
    ...
    # --- Resample Curve (fixed point count before storing attributes) ---
    resample_curve = nodes.new('GeometryNodeResampleCurve')
    resample_curve.mode = 'COUNT'
    resample_curve.location = (-1000, 150)
    ...
    named_attr_radius.location = (150, -950)

    switch_radius = nodes.new('GeometryNodeSwitch')
    switch_radius.input_type = 'FLOAT'
    switch_radius.inputs['False'].default_value = 1.0
    switch_radius.location = (400, -950)
    ...
    links.new(group_in.outputs['Geometry'], resample_curve.inputs['Curve'])
    links.new(group_in.outputs['Resample Count'], resample_curve.inputs['Count'])
    links.new(resample_curve.outputs['Curve'], store_u.inputs['Geometry'])
    ...
    # 7b. Radius Attribute & Switch logic (Scale selection)
    links.new(named_attr_radius.outputs['Exists'], switch_radius.inputs['Switch'])
    links.new(named_attr_radius.outputs[0], switch_radius.inputs['True']) # [0] is 'Attribute'
    links.new(switch_radius.outputs['Output'], curve_to_mesh.inputs['Scale'])
...
def sync_modifier_inputs(mod, node_group):
    """Push the node group's interface default values into the modifier's
    own input properties. Blender does NOT do this automatically for
    modifiers that already exist / were created before the defaults changed,
    so we set them explicitly via each socket's identifier."""
    skip_names = {"Geometry", "Material"}
    for item in node_group.interface.items_tree:
        ...
        try:
            ...
            print(f"  (Konnte '{item.name}' nicht setzen: {e})")

    # Blender braucht das, damit die UI/Depsgraph den neuen Wert übernimmt
    mod.node_group.interface_update(bpy.context) if hasattr(mod.node_group, "interface_update") else None
...
def apply_to_selected_object():
    ...
    if GROUP_NAME not in obj.modifiers:
        ...
        mod = obj.modifiers[GROUP_NAME]
        print(f"Updated '{GROUP_NAME}' on '{obj.name}'.")

    sync_modifier_inputs(mod, node_group)
